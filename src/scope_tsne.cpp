...
void TSNE::run(double* X, int N, int D, double* Y,
    ...
    */
    ...
    for (int iter = 0; iter < max_iter; iter++) {
        ...
        for (int i = init_from_Y * no_dims; i < N * no_dims; i++) {
            // Update gains
        ...
        zeroMean(Y, N, no_dims, init_from_Y);
...
void TSNE::computeGaussianPerplexity(double* X, int N, int D, int** _row_P, int** _col_P, double** _val_P, double perplexity, int K, int verbose) {
    ...
    */
    ...
    tsne::VpTree<double> *tree = new VpTree<double>(N, D, X);
    ...
    for (int n = 0; n < N; n++)
    {
        ...
        tree->search(X + n, K + 1, indices, distances);
...
void TSNE::zeroMean(double *X, int N, int D, int init_from_Y){
